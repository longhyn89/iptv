...
function getUrlCategories() {
  return BASEAPI + "/the-loai";
}

function getUrlCountries() {
  return BASEAPI + "/quoc-gia";
}

function getUrlYears() {
  return BASEAPI + "/nam-phat-hanh";
}
...
function parseMovieDetail(apiResponseJson) {
  try {
    ...
    var categories = (movie.category || []).map(function(c) {
      return c ? c.name : "";
    }).filter(Boolean).join(", ");
    var countries = (movie.country || []).map(function(c) {
      return c ? c.name : "";
    }).filter(Boolean).join(", ");
